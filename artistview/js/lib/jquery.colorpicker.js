...
;(function ($) {
	...
	var _colorpicker_index = 0,
        ...
        IE8 = (navigator.userAgent.toLowerCase().indexOf('msie 8') != -1) ? true : false,
		...
		this.parts = {
			...
			map: function (inst) {
				...
				_html = function () {
					var html = '<div class="ui-colorpicker-map ui-colorpicker-border">'
							...
							+ (inst.options.alpha && !IE8 ? '<span class="ui-colorpicker-map-layer-alpha">&nbsp;</span>' : '')
			...
			preview: function (inst) {
				...
				_html = function () {
                    return '<div class="ui-colorpicker-preview ui-colorpicker-border">'
                        + '<div class="ui-colorpicker-close"></div>'
                        + '<div class="ui-colorpicker-preview-current-text">new</div>'
                        + '<div class="ui-colorpicker-preview-current">'
                        + (!IE8 ? '<div class="ui-colorpicker-preview-current-alpha"></div>' : '')
                        + '</div>'
                        + '<div class="ui-colorpicker-preview-initial">'
                        + (!IE8 ? '<div class="ui-colorpicker-preview-initial-alpha"></div>' : '')
                        + '</div>'
                        + '<div class="ui-colorpicker-preview-initial-text">current</div>'
                        + '</div>';
				};
				...
				this.init = function () {
                    ...
                    if (IE8) $('<div id="ie_bg"></div>').insertBefore('#customize-design2');

                    $('.ui-colorpicker-preview-initial').attr('data-opacity', inst.color.getAlpha());

                    if (!IE8) {
                        $('.ui-colorpicker-preview-initial-alpha').css('opacity', 1 - inst.color.getAlpha());
                    } else {
                        $('.ui-colorpicker-preview-initial').css('opacity', inst.color.getAlpha());
                    }

                    $('.ui-colorpicker-close').click(function() {
                        $('#colorpicker_area').fadeOut();
                    });

					$('.ui-colorpicker-preview-initial', e).click(function () {
                        var opacity = $(this).data('opacity');
						inst.color = inst.currentColor.copy();
                        inst.color.setAlpha(opacity);
						inst._change(inst.color.set);
                        $('#colorpicker_'+ inst.options.uid +'_opacity').val(opacity);
                        $('#colorpicker_'+ inst.options.uid +'_opacity').parent().find('b').css('opacity', opacity);
                        $('.opacity_scroller').css('left', opacity * 200);
					});
				};
				...
				this.repaint = function () {
                    ...
                    //$('.ui-colorpicker-preview-initial-alpha', e).css('opacity', 1 - inst.color.getAlpha());
                    ...
                    if (IE8) {
                        $('.ui-colorpicker-map').css('opacity', inst.color.getAlpha());
                        //$('.ui-colorpicker-bar-layer-1').css('opacity', inst.color.getAlpha());
                        $('.ui-colorpicker-preview-current').css('opacity', inst.color.getAlpha());
                    }
				};
			},
			...
			rgb: function (inst) {
				...
				_html = function () {
					...
					if (inst.options.rgb) {
						html += '<div class="ui-colorpicker-rgb-r"><input class="ui-colorpicker-mode" type="radio" value="r"/><label>' + inst._getRegional('rgbR') + '</label><input class="ui-colorpicker-number" min="0" max="255"/></div>'
							+ '<div class="ui-colorpicker-rgb-g"><input class="ui-colorpicker-mode" type="radio" value="g"/><label>' + inst._getRegional('rgbG') + '</label><input class="ui-colorpicker-number" min="0" max="255"/></div>'
							+ '<div class="ui-colorpicker-rgb-b"><input class="ui-colorpicker-mode" type="radio" value="b"/><label>' + inst._getRegional('rgbB') + '</label><input class="ui-colorpicker-number" min="0" max="255"/></div>';
					}
			...
			alpha: function (inst) {
				...
				_html = function () {
					...
					if (inst.options.alpha) {
						html += '<div class="ui-colorpicker-a"><div class="ttl">Opacity</div><div class="left"><div class="opacity_scrollarea"><div class="opacity_scroller_area"><div class="opacity_scroller"></div></div></div></div><div class="right"><input class="ui-colorpicker-mode" name="mode" type="radio" value="a"><label>' + inst._getRegional('alphaA') + '</label><span class="ui-colorpicker-unit">%</span><input class="ui-colorpicker-number" min="0" max="100"></div></div>';
					}
				...
				this.init = function () {
                    ...
                    function setPercents(percents) {
                        var input = $('.ui-colorpicker-a').find('.ui-colorpicker-number');
                        input.val(parseInt(percents));
                        inst.color.setAlpha(percents / 100);
                        inst._change();
                        var id = inst.options.uid;
                        $('#colorpicker_'+ id +'_opacity').parent().find('b').css('opacity', percents / 100);
                    }

                    function percentsInit() {
                        var id = inst.options.uid;
                        //console.log(id);
                        var opacity = $('#colorpicker_'+ id +'_opacity').val();
                        $('.ui-colorpicker-a').find('.opacity_scroller').css('left', opacity * 200);
                        setPercents(opacity * 100);
                        $('#colorpicker_'+ id +'_opacity').parent().find('b').css('opacity', opacity);
                    }

                    percentsInit();

                    $('.opacity_scroller').draggable({
                        containment: 'parent',
                        drag: function(e) {
                            var _this = e.target;
                            var left = parseInt($(_this).css('left'));
                            var percents = (left === 102 ) ? parseInt((left - 2) / 2) : parseInt(left / 2);
                            setPercents(percents);
                            var id = inst.options.uid;
                            $('#colorpicker_'+ id +'_opacity').val(percents / 100);
                        }
                    });
					...
					$('.ui-colorpicker-number', e).bind('change keyup', function () {
                        var value = $('.ui-colorpicker-a .ui-colorpicker-number', e).val() / 100;
						inst.color.setAlpha(value);
                        $('.ui-colorpicker-a').find('.opacity_scroller').css('left', value * 200);
						inst._change();
					});
				};
	...
	$.widget("vanderlee.colorpicker", {
		...
		_generatePopup: function() {
            ...
            $('#colorpicker').html('');
			$('#colorpicker').append(_container_popup);
			...
			$(document).delegate('html', 'touchstart click', function (event) {
                ...
                if ($(event.target).attr('class') === 'colorpicker_input') {
