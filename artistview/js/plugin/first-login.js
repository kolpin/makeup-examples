define(['jquery', 'placeholder'], function ($) {
    return {
        ...
        init: function () {
            if (!(this.node = $('#first-login')).size() && !(this.node = $('#add-product')).size() && !(this.node = $('.product-category')).size() && !(this.node = $('#create-event')).size() && !(this.node = $('#group-create')).size()) return;

            $.Placeholder.init({ color : "#aaa" });
            this.whoYouAre = this.node.find('.who-are-you').find('label').on('click touchend', $.proxy(this.onWhoAreYou, this));
